...
- `publish-chat-knowledge`: publishes the editorial chat knowledge artifact to Cloudflare R2.
...
Responsible for publishing the canonical cloud copy of the editorial chat knowledge generated by `portfolio`.

Expected responsibilities:

- accept the editorial artifact generated from `.generated/chat/knowledge.json`
- validate the payload shape already consumed by `portfolio-api`
- wrap it with cloud publication metadata
- upload the canonical object to Cloudflare R2
...
`publish-chat-knowledge` accepts either:

- the raw editorial artifact generated by `portfolio`
- a wrapped payload with `artifact`, optional `release`, and optional `source`

The source artifact generated by `portfolio` currently follows this shape:
...
The canonical cloud object published by `portfolio-cloud` wraps that payload as:
...
This keeps `portfolio` as the editorial source of truth while making `portfolio-cloud` the owner of the canonical cloud handoff in R2.
...
- `publish-chat-knowledge` deployed as an internal Lambda for editorial artifact handoff to R2
